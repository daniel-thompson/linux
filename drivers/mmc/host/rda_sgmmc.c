...
#if (_TGT_AP_MEM_SIZE > 256)
#define RDA_MMC_REQ_NUM_PAGE	(16)
#else
#define RDA_MMC_REQ_NUM_PAGE	(4)
#endif /* _TGT_AP_MEM_SIZE > 256 */
#else
#define RDA_MMC_REQ_NUM_PAGE	(16)
				...
				struct mmc_command *cmd, struct mmc_data *data)
{
	...
	if (cmd->opcode == MMC_READ_SINGLE_BLOCK) {
		...
		if (host->ddr_mode)
			configReg |= SDMMC_SAMPLE_EDGE_SEL_FALL_EN;
...
static u32 hal_get_op_status2(struct rda_mmc_host *host)
{
	HWP_SDMMC_T *hwp_sdmmc = (HWP_SDMMC_T*)host->base;
	return hwp_sdmmc->SDMMC_STATUS2;
}
#else
static u32 hal_get_op_status2(struct rda_mmc_host *host)
{
				...
				HAL_SDMMC_TRANSFER_T* transfer)
{
	...
	xfer_size = transfer->blockNum * transfer->blockSize;
	/*
	 * MMC host controller can use the max address is 32MB,
	 * so we do a sanity checking.
	 */
	...
	switch (transfer->direction){
		case HAL_SDMMC_DIRECTION_READ:
			transfer->ifc_config.request = HAL_IFC_SDMMC_RX + host->id * 2;
			break;

		case HAL_SDMMC_DIRECTION_WRITE:
			transfer->ifc_config.request = HAL_IFC_SDMMC_TX + host->id *2;
			break;
	...
	for_each_sg(transfer->ifc_config.sg, s, transfer->ifc_config.sg_len, i)  {
		if (!IS_ALIGNED(sg_phys(s), sizeof(unsigned int))) {
			dev_err(mmc_dev(host->mmc),
				"can not support unaligned 32-bits address(0x%x).\n",
				sg_phys(s));
			return -EILSEQ;
		}
	}

	ifc_ch = ifc_transfer_sg_start(&transfer->ifc_config, xfer_size, HAL_IFC_SIZE_32_MODE_MANUAL);
	if (ifc_ch == HAL_UNKNOWN_CHANNEL){
		dev_err(mmc_dev(host->mmc), "transfer start with invalide channel\n");
		return -EILSEQ;
	}
	else{
		transfer->ifc_config.channel = ifc_ch;
				...
				HAL_SDMMC_TRANSFER_T* transfer)
{
	...
	rda_dbg_mmc("stop channel %d\n", (int)transfer->ifc_config.channel);

	if (transfer->ifc_config.channel == HAL_UNKNOWN_CHANNEL
		|| transfer->ifc_config.channel >= SYS_IFC_STD_CHAN_NB) {
		dev_err(mmc_dev(host->mmc),
			"hal_data_transfer_stop invalide channel %d \n",
			transfer->ifc_config.channel);
		return;
	}
	...
	if (host->present && data && (data->flags & MMC_DATA_READ)) {
		...
		ifc_transfer_flush(transfer->ifc_config.request, transfer->ifc_config.channel);
	}

	if (data) {
		ifc_transfer_sg_stop(&transfer->ifc_config);
	}
	transfer->ifc_config.channel = HAL_UNKNOWN_CHANNEL;
	transfer->ifc_config.request = HAL_IFC_NO_REQWEST;
}
...
static int do_data_abort(struct rda_mmc_host *host)
{
	...
	HAL_SDMMC_TRANSFER_T *transfer = &host->data_transfer;
	struct mmc_data *data = host->data;
	u32 sta = (hal_get_op_status(host)).reg;
	u32 sta2 = hal_get_op_status2(host);
	u32 count = ifc_transfer_sg_get_tc(&transfer->ifc_config);

	if (data->flags & MMC_DATA_READ) {
		dev_err(mmc_dev(host->mmc), "%s: read: int = 0x%x, sta = 0x%x, sta2 = 0x%x, "
			"ch = %d, tc = 0x%x, blknum = %d, blksize = %d\n",
			__func__, host->data_err, sta, sta2,
			transfer->ifc_config.channel, count,
			transfer->blockNum, transfer->blockSize);
	} else if (data->flags & MMC_DATA_WRITE){
		dev_err(mmc_dev(host->mmc), "%s: write: int = 0x%x, sta = 0x%x, sta2 = 0x%x, "
			"ch = %d, tc = 0x%x, blknum = %d, blksize = %d\n",
			__func__, host->data_err, sta, sta2,
			transfer->ifc_config.channel, count,
			transfer->blockNum, transfer->blockSize);
...
static int do_data_complete(struct rda_mmc_host *host)
{
	...
	if (!data || transfer->ifc_config.channel == HAL_UNKNOWN_CHANNEL
		|| transfer->ifc_config.channel >= SYS_IFC_STD_CHAN_NB) {
		dev_err(mmc_dev(host->mmc), "do_data_complete invalid channel(%d), flags = 0x%x\n",
			transfer->ifc_config.channel, (data ? data->flags : 0));
		return -EREMOTEIO;
	}

	if (ifc_transfer_sg_get_tc(&transfer->ifc_config) == 0) {
		ifc_transfer_sg_stop(&transfer->ifc_config);
		rda_dbg_mmc("release channel %d\n", (int)transfer->ifc_config.channel);
		transfer->ifc_config.channel = HAL_UNKNOWN_CHANNEL;
	}
	...
	if (cmd->flags & MMC_RSP_PRESENT){
		...
		if (result) {
			...
			dev_err(mmc_dev(host->mmc),
				...
				cmd->opcode, (int)host->data_transfer.ifc_config.channel, result);
	...
	struct mmc_data *data)
{
	...
	rda_dbg_mmc("%s, [mmc%d]: cmd%d, sg_len = %d, blks = %d, blksz = %d\n",
		__func__, host->id, cmd->opcode, data->sg_len, data->blocks, data->blksz);

	transfer->ifc_config.sg = data->sg;
	transfer->ifc_config.sg_len = data->sg_len;
	...
	if (timeout == 0) {
		...
		u32 op_status2 = hal_get_op_status2(host);
		u32 count = ifc_transfer_sg_get_tc(&transfer->ifc_config);
		...
		if (data->flags & MMC_DATA_READ) {
			dev_err(mmc_dev(host->mmc),
				...
				"op_sta = 0x%x, op_sta2 = 0x%x, "
				"ifc = %d, cnt = 0x%x\n",
				MCD_DATA_TIMEOUT_MS, cmd->opcode,
				data->blocks, irq, op_status, op_status2,
				transfer->ifc_config.channel, count);
		} else if (data->flags & MMC_DATA_WRITE) {
			dev_err(mmc_dev(host->mmc),
		 ...
		 */
			do {
				...
				sta_cmd.opcode = MMC_SEND_STATUS;
				sta_cmd.arg = mmc->card->rca << 16;
				sta_cmd.flags = MMC_RSP_SPI_R2 | MMC_RSP_R1 | MMC_CMD_AC;
				sta_cmd.data = NULL;

				do_command(host->mmc, &sta_cmd, NULL);
				dev_info(mmc_dev(host->mmc), "timeout: dev status = 0x%x\n", sta_cmd.resp[0]);
				...
				struct mmc_data *data)
{
	...
	host->data = data;
	ret = __do_data_transfer(mmc, cmd, data);
	host->data = NULL;
...
static void rda_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	switch (ios->power_mode) {
		case MMC_POWER_OFF:
			if (host->mmc_pm != MMC_POWER_OFF && host->id == 0) {
				...
				/* Waiting 40ms until power is completely turned off. */
				mdelay(40);
			}
	...
	if (host->clock != ios->clock) {
		...
		if (host->clock) {
			...
			hwp_sdmmc->SDMMC_TRANS_SPEED = SDMMC_TRANS_SPEED(clk_div);
			hwp_sdmmc->SDMMC_MCLK_ADJUST = SDMMC_MCLK_ADJUST(host->mclk_adj);
			if (host->clk_inv) {
				hwp_sdmmc->SDMMC_MCLK_ADJUST |= SDMMC_CLK_INV;
			}

			dev_info(mmc_dev(host->mmc),
				"set clk = %d, bus_clk = %d, divider = %d, adj_reg = 0x%x\n",
				(int)host->clock, (int)mclk, (int)clk_div,
	...
	if (host->timing != ios->timing) {
		...
		host->ddr_mode = (ios->timing == MMC_TIMING_UHS_DDR50) ? true : false;
	}
...
static int rda_mmc_probe(struct platform_device *pdev)
{
	 ...
	 */
	mmc->max_req_size = RDA_MMC_REQ_NUM_PAGE * PAGE_SIZE;
	mmc->max_seg_size = RDA_MMC_REQ_NUM_PAGE * PAGE_SIZE;
	...
	*/
	...
	dev_info(&pdev->dev, "rda sg mmc.%d initialized.\n", i);
